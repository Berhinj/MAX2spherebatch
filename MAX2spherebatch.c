...
SPHERE_LLTABLE *sphere_lltable = NULL;
typedef struct {
   UV uv;
   short int face;
} PERSPECTIVE_LLTABLE;
PERSPECTIVE_LLTABLE *perspective_lltable = NULL;
int ntable = 0;
int itable = 0;
...
int main(int argc,char **argv)
{
	...
	int is_perspective;
	char fname1[256],fname2[256],sphere_tablename[256], perspective_tablename[256];
	...
	BITMAP4 *frame1 = NULL,*frame2 = NULL,*spherical = NULL,*perspective = NULL;
	...
	for (i=1;i<argc-1;i++) {
		if (strcmp(argv[i],"-w") == 0) {
			params.sphere_outwidth = atoi(argv[i+1]);
				params.sphere_outwidth = 4 * (params.sphere_outwidth / 4);    // Make factor of 4
				params.sphere_outheight = params.sphere_outwidth / 2;         // Will be even
				params.perspective_outwidth = params.sphere_outwidth / 4;    // Make factor of 4
				params.perspective_outheight = params.sphere_outwidth / 4;         // Will be even
	...
	if (params.sphere_outwidth < 0) {
		params.sphere_outwidth = template[whichtemplate].equiwidth;
		params.sphere_outheight = params.sphere_outwidth / 2;
	}
	spherical = Create_Bitmap(params.sphere_outwidth,params.sphere_outheight);

	if (params.perspective_outwidth < 0) {
		params.perspective_outwidth = template[whichtemplate].centerwidth;
   		params.perspective_outheight = template[whichtemplate].height;
	}
   	perspective = Create_Bitmap(params.perspective_outwidth,params.perspective_outheight);

	if (frame1 == NULL || frame2 == NULL || spherical == NULL || perspective == NULL) {
		fprintf(stderr,"%s() - Failed to malloc memory for the images\n",argv[0]);
		exit(-1);
	}

	// Does a spherical lltable exist? If it does load it, if not create it and save it
	ntable = params.sphere_outheight * params.sphere_outwidth * params.antialias * params.antialias;
	sphere_lltable = malloc(ntable*sizeof(SPHERE_LLTABLE));
	sprintf(sphere_tablename,"%d_%d_%d_%d.sphere_lltable",whichtemplate,params.sphere_outwidth,params.sphere_outheight,params.antialias);
	if ((fptr = fopen(sphere_tablename,"r")) != NULL) {
		if (params.debug){
			fprintf(stderr,"%s() - Reading Sphere lookup table\n",argv[0]);
		}
		if ((n = fread(sphere_lltable,sizeof(SPHERE_LLTABLE),ntable,fptr)) != ntable) {
			fprintf(stderr,"%s() - Failed to read Sphere lookup table \"%s\" (%d != %d)\n",argv[0],sphere_tablename,n,ntable);
		}
		fclose(fptr);
	}
	if (n != ntable) {
		if (params.debug){
			fprintf(stderr,"%s() - Generating Sphere lookup table\n",argv[0]);
		}

		dx = params.antialias * params.sphere_outwidth;
		dy = params.antialias * params.sphere_outheight;
		itable = 0;
		for (j=0;j<params.sphere_outheight;j++) {
			y0 = j / (double)params.sphere_outheight;
			for (i=0;i<params.sphere_outwidth;i++) {
				x0 = i / (double)params.sphere_outwidth;
				for (aj=0;aj<params.antialias;aj++) {
					...
					for (ai=0;ai<params.antialias;ai++) {
						...
						sphere_lltable[itable].face = SphericalFindFaceUV(longitude,latitude,&(sphere_lltable[itable].uv));
						itable++;
						}
					}
				}
	...
	if (params.debug)
		fprintf(stderr,"%s() - Saving Sphere lookup table\n",argv[0]);
		fptr = fopen(sphere_tablename,"w");
		fwrite(sphere_lltable,ntable,sizeof(SPHERE_LLTABLE),fptr);
		fclose(fptr);
	}
	// Does a perspective lltables exist? If they do, load them, if not create them and save them
	ntable = params.perspective_outheight * params.perspective_outwidth * params.antialias * params.antialias * 6;
	perspective_lltable = malloc(ntable*sizeof(PERSPECTIVE_LLTABLE));
	sprintf(perspective_tablename,"%d_%d_%d_%d.perspective_lltable",whichtemplate,params.perspective_outwidth,params.perspective_outheight,params.antialias);
	if ((fptr = fopen(perspective_tablename,"r")) != NULL) {
		if (params.debug){
			fprintf(stderr,"%s() - Reading Perspective lookup table\n",argv[0]);
		}
		if ((n = fread(perspective_lltable,sizeof(PERSPECTIVE_LLTABLE),ntable,fptr)) != ntable) {
			fprintf(stderr,"%s() - Failed to read Perspective lookup table \"%s\" (%d != %d)\n",argv[0],perspective_tablename,n,ntable);
		}
		fclose(fptr);
	}
	if (n != ntable) {
		if (params.debug){
			fprintf(stderr,"%s() - Generating Perspective lookup table\n",argv[0]);
		}
		dx = params.antialias * params.perspective_outwidth;
		dy = params.antialias * params.perspective_outheight;

		itable = 0;
		for (int f=0;f<6;f++) {
			for (j=0;j<params.perspective_outheight;j++) {
				for (i=0;i<params.perspective_outwidth;i++) {
					for (aj=0;aj<params.antialias;aj++) {
						y = (j + aj/(double)params.antialias) / (double)params.perspective_outheight; // -0.5 ... 0.5
						for (ai=0;ai<params.antialias;ai++) {
		           			x = (i + ai/(double)params.antialias) / (double)params.perspective_outwidth; // 0 ... 1
							perspective_lltable[itable].face = PerspectiveFindFaceUV(f,x,y,&(perspective_lltable[itable].uv));
							itable++;
						}
					}
				}
	...
	if (params.debug)
		fprintf(stderr,"%s() - Saving Perspective lookup table\n",argv[0]);
		fptr = fopen(perspective_tablename,"w");
		fwrite(perspective_lltable,ntable,sizeof(PERSPECTIVE_LLTABLE),fptr);
		fclose(fptr);
	}
	...
	for (nframe=params.nstart;nframe<=params.nstop;nframe++) {
		...
		if (params.debug)
			...
			// Erase_Bitmap(spherical,params.sphere_outwidth,params.sphere_outheight,black);
		...
		// Build the perspective images
		starttime0 = GetRunTime();
		
		is_perspective = 1;
	    const char *fs[6] = { "L", "R", "U", "F", "B", "D"};
		itable = 0;
				...
				for (i=0;i<params.perspective_outwidth;i++) {
					...
					for (aj=0;aj<params.antialias;aj++) {
						...
						for (ai=0;ai<params.antialias;ai++) {
							...
							face = perspective_lltable[itable].face;
							uv = perspective_lltable[itable].uv;
							itable++;
			

							c = GetColour(face,is_perspective,uv,frame1,frame2);
							csum.r += c.r;
							csum.g += c.g;
							csum.b += c.b;
					...
					index = j * params.perspective_outwidth + i; 
					perspective[index].r = csum.r / params.antialias2;
					perspective[index].g = csum.g / params.antialias2;
					perspective[index].b = csum.b / params.antialias2;

				}
			}
			if (params.debug){
				fprintf(stderr,"%s() - Writing %s perspective\n",argv[0], fs[f]);
			}
			WritePerspective(fname1,nframe,fs[f],perspective,params.perspective_outwidth,params.perspective_outheight);
		}
		if (params.debug){
			fprintf(stderr,"%s() - Perspective Processing time: %g seconds\n",argv[0],GetRunTime()-starttime0);
		}
		...
		for (j=0;j<params.sphere_outheight;j++) {
			//y0 = j / (double)params.sphere_outheight;
			//if (params.debug && j % (params.sphere_outheight/32) == 0)
			...
			for (i=0;i<params.sphere_outwidth;i++) {
				...
				for (aj=0;aj<params.antialias;aj++) {
					...
					for (ai=0;ai<params.antialias;ai++) {
						...
						//longitude = x * TWOPI - M_PI;    // -pi ... pi
						//latitude = y * M_PI - M_PI/2;    // -pi/2 ... pi/2
						face = sphere_lltable[itable].face;
						uv = sphere_lltable[itable].uv;
						itable++;
				...
				// Finally update the spherical image
				index = j * params.sphere_outwidth + i; 
		...
		WriteSpherical(fname1,nframe,spherical,params.sphere_outwidth,params.sphere_outheight);

		if (params.debug){
			fprintf(stderr,"%s() - Sphere Processing time: %g seconds\n",argv[0],GetRunTime()-starttime1);
		}
	}
...
/*
   Write perspective image
	The file name is either params.outfilename or based upon the basename provided
*/
int WritePerspective(char *basename,int nframe,const char *suffix,BITMAP4 *img,int w,int h)
{
	int i;
   	FILE *fptr;
	char fname[256];
	...
	if (strlen(params.outfilename) < 1) {
		...
		strcat(out,"_%s.jpg");
		sprintf(fname,out,nframe, suffix);
	}
...
//    strcat(fname,".jpg");
	...
	// Save
	if ((fptr = fopen(fname,"wb")) == NULL) {
		fprintf(stderr,"WritePerspective() - Failed to open output file \"%s\"\n",fname);
		return(FALSE);
	}
	JPEG_Write(fptr,img,w,h,100);
	fclose(fptr);
...
*/
int PerspectiveFindFaceUV(int face,double x,double y,UV *uv)
{
	int found = 1;
	UV fuv;
	double fourdivpi = 4.0 / M_PI;

	// This is required as the gopro track0/track5 are not suited for cubemap asis, a deformation is required
	// pi/4 = atan(1) !!! 4/pi is actually a way to normalize angles ranging from -pi/4 to pi/4
	// these angles are being obtained by atan(x) x being cube coordinnates
	// I believe this deformation is needed to take into account the "image/camera" focal/curvature
	fuv.u = ((atan((x*2)-1) * fourdivpi) + 1) * 0.5;
	fuv.v = ((atan((y*2)-1) * fourdivpi) + 1) * 0.5;
	...
	if (fuv.u < 0 || fuv.v < 0 || fuv.u >= 1 || fuv.v >= 1) {
		fprintf(stderr,"PerspectiveFindFaceUV() - Illegal (u,v) coordinate (%g,%g) on face %d\n",fuv.u,fuv.v,found);
		return(-1);
	}
...
/*
   ...
   Return -1 if something went wrong, shouldn't
*/
int SphericalFindFaceUV(double longitude,double latitude,UV *uv)
{
    int k,found = -1;
...
*/
BITMAP4 GetColour(int face,int is_perspective,UV uv,BITMAP4 *frame1,BITMAP4 *frame2)
{
	int ix,iy,index;
	int x0,w;
	double alpha,duv;
...
/*
	Rotate a uv by 270 degrees counterclockwise
*/
void RotateUV270(UV *uv)
{
	UV tmp;

	tmp = *uv;
	uv->v = tmp.u;
	uv->u = NEARLYONE - tmp.v;
}
...
void Init(void)
{
	params.sphere_outwidth = -1;
	params.sphere_outheight = -1;
    params.perspective_outwidth = -1;
    params.perspective_outheight = -1;
	params.framewidth = -1;
